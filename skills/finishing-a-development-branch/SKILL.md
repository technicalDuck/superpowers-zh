...
| 状态 | 菜单 | 清理 |
|------|------|------|
| `GIT_DIR == GIT_COMMON` 且 `BRANCH` 是 `main/master` | 主分支当前工作区选项 | 无 worktree 可清理 |
| `GIT_DIR == GIT_COMMON` 且 `BRANCH` 不是 `main/master` | 标准 4 个选项 | 无 worktree 可清理 |
...
如果当前分支已经是 `main` 或 `master`，跳过基础分支确认。当前工作已经在主分支上，不要展示"合并回主分支"选项。

否则，或者询问："这个分支是从 main 分出来的——对吗？"

### 步骤 4：展示选项

**普通仓库主分支 —— 准确展示以下 3 个选项：**

```
实现已完成，当前就在 <branch> 分支。你想怎么做？

1. 保持当前分支现状（我稍后处理提交/推送）
2. 推送当前分支
3. 丢弃未提交的更改
...
**普通仓库功能分支和命名分支 worktree —— 准确展示以下 4 个选项：**
...
**只对功能分支选项 1 和 4 执行。** 主分支选项、功能分支选项 2 和 3 始终保留当前工作区。
...
- **问题：** "接下来该做什么？" → 含糊不清
- **修复：** 准确展示结构化选项（主分支和分离 HEAD 时是 3 个，功能分支时是 4 个）

**主分支上展示合并选项**

- **问题：** 当前已经在 `main/master`，还提示"合并回主分支"
- **修复：** 主分支场景展示保持现状、推送、丢弃未提交更改三个选项
...
- 在提供选项前验证测试
- 展示菜单前检测环境
- 根据当前环境展示正确菜单：主分支 3 个选项，功能分支 4 个选项，分离 HEAD 3 个选项
- 丢弃类选项要求输入确认
- 只在功能分支选项 1 和 4 时清理 worktree
